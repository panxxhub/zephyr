...
static int build_bd_ring(const struct device *dev, uint32_t channel)
{
	...
	if (channel == CH_TX && ch->tx_src_addr != 0) {
		...
		/* Large transfer: split across multiple BDs.
		 * Each BD gets up to bd_buf_bytes (capped at len_mask).
		 * SOF on first, EOF on last. */
		...
		for (uint32_t i = 0; i < num_needed; i++) {
			...
			ch->bds[i].control = chunk & len_mask;
			if (i == 0) {
				ch->bds[i].control |= BD_CTRL_SOF;
			}
			if (i == num_needed - 1) {
				ch->bds[i].control |= BD_CTRL_EOF;
			}
			for (int a = 0; a < 5; a++) {
