...
static int wait_xfr_intr_complete(const struct device *dev, uint32_t time_out)
{
	...
	events = k_event_wait(&emmc->irq_event,
			      ZYNQ_SDHC_HOST_XFER_COMPLETE | ERR_INTR_STATUS_EVENT(ZYNQ_SDHC_HOST_DMA_TXFR_ERR), false,
			      wait_time);
...
static int zynq_sdhc_config_init(const struct device *dev)
{
	volatile struct zynq_sdhc_reg *regs = DEV_REG(dev);
	struct zynq_sdhc_data *data = dev->data;
...
#define ZYNQ_SDHC_INIT(n)                                                                                              \
	...
	{                                                                                                              \
		return zynq_sdhc_config_init(dev);                                                                     \
	}                                                                                                              \
	DEVICE_DT_INST_DEFINE(n, zynq_sdhc_##n##_init, NULL, NULL, NULL, POST_KERNEL,                                  \
