...
static int uart_xlnx_ps_wq_init(void)
{
	...
	k_work_queue_start(&uart_xlnx_work_q, uart_xlnx_stack,
			   K_THREAD_STACK_SIZEOF(uart_xlnx_stack), CONFIG_UART_XLNX_WQ_PRIORITY,
			   NULL);
	return 0;
}
...
static void uart_xlnx_irq_cb_handler(struct k_work *work)
{
	struct uart_xlnx_ps_dev_data_t *dev_data =
		CONTAINER_OF(work, struct uart_xlnx_ps_dev_data_t, irq_cb_work);
